(function(root,factory){
  ...
  function checks(q){if(q.type==='journal'){const d=(q.answer?.debit||[]).reduce((s,r)=>s+(Number(r.amount)||0),0),c=(q.answer?.credit||[]).reduce((s,r)=>s+(Number(r.amount)||0),0);return[{label:'借方合計と貸方合計が一致',expected:d===c?`${num(d)} = ${num(c)}`:'不一致',evidenceRefs:['answer.debit','answer.credit']}];}const locs=locations(q),out=Object.entries(q.answer?.cells||{}).map(([id,v])=>({label:`${cellLabel(q,id,locs.get(id))}を正答値と照合`,expected:num(v),evidenceRefs:[`answer.cells.${id}`]}));if(q.format==='bookkeeping-voucher-entry'){out.unshift({label:'取引ごとに現金の動きと伝票の種類を対応づける',expected:'現金の受取＝入金伝票 / 現金の支払＝出金伝票 / 現金を伴わない取引＝振替伝票',evidenceRefs:['table.rows']});return out;}if(q.type==='ledger')out.unshift({label:'上から順に残高または帳簿値を更新',expected:'各行の結果を次の行の計算へ引き継ぐ',evidenceRefs:['table.rows']});return out;}
