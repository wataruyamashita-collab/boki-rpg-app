...
function reachableLocks(root){let commits=[];try{const output=git(root,['log','--reverse','--format=%H','HEAD','--',LOCK_PATH]);commits=output?output.split('\n'):[];}catch(_){return [];}return commits.flatMap(commit=>{try{return [{commit,lock:JSON.parse(git(root,['show',`${commit}:${LOCK_PATH}`]))}];}catch(_){return [];}});}
...
function verify(root){const absolute=path.join(root,LOCK_PATH),errors=[];if(!fs.existsSync(absolute))return {ok:false,errors:['lock missing']};let lock;try{lock=JSON.parse(fs.readFileSync(absolute,'utf8'));}catch(_){return {ok:false,errors:['lock invalid JSON']};}
  ...
  const history=reachableLocks(root),historicalAuthority=history.find(item=>isFinal(item.lock));
  const migrationAuthorized=process.env.AUDIT_LOCK_ALLOW_PENDING_BASELINE==='1'&&history.length>0;
  if(!historicalAuthority){if(!migrationAuthorized)errors.push('historical baseline not reachable from HEAD');}
  else if(canonical(identityPayload(historicalAuthority.lock))!==canonical(identityPayload(lock))||historicalAuthority.lock.baselineIdentity!==lock.baselineIdentity)errors.push('historical baseline mismatch');
