...
function validateEvidence({authorities,final,rows,gateReports,integrity,currentSourceHashes}){
  ...
  const legalSequences=[[2],[2,3],[2,3,4],[2,3,4,5],[2,3,4,5,6],[2,3,4,5,6,7],[2,3,4,5,6,7,8],[2,3,4,5,6,7,8,9]];
  assert(legalSequences.some(sequence=>stable(sequence)===stable(generations)),'supported successor sequence must be contiguous from Generation 2 through at most Generation 9');
