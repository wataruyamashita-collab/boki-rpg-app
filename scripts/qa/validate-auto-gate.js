...
function validateEvidence({authorities,final,rows,gateReports,integrity,currentSourceHashes}){
  ...
  const legalSequences=[[2],[2,3],[2,3,4],[2,3,4,5],[2,3,4,5,6]];
  assert(legalSequences.some(sequence=>stable(sequence)===stable(generations)),'supported successor sequence must be exactly [2], [2,3], [2,3,4], [2,3,4,5], or [2,3,4,5,6]');
