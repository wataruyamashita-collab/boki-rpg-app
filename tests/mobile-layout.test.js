...
assert(/width:\s*9ch/.test(numericInputRule) && /min-width:\s*9ch/.test(numericInputRule) && /max-width:\s*9ch/.test(numericInputRule) && /font-variant-numeric:\s*tabular-nums/.test(numericInputRule), 'editable numeric controls expose the nine-character canonical maximum without cyclic 100% table sizing');
