...
assert(localAssets.length > 0 && localAssets.every(([, , query]) => query === '?v=20260821-2'), 'すべてのローカルCSS/JSに最新のキャッシュバスターを付ける');
...
assert(viewSource.includes("input.setAttribute('pattern', '[0-9]*')"), 'iPhoneで金額欄に数字キーパッドを確実に表示する');
...
assert(/@media \(max-width: 480px\)[\s\S]*?select,\s*\.amount-input\s*{[^}]*font-size:\s*16px/s.test(cssSource), 'iPhoneで入力時の意図しない自動ズームを防ぐ');
...
assert(html.includes('css/style.css?v=20260821-2') && html.includes('js/view.js?v=20260821-2'), 'iPhone Chromeに改修後のCSSとJSを再読み込みさせる');
