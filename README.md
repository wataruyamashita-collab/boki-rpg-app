...
`npm run typecheck` は、JavaScriptのJSDocと型宣言を確認する任意のローカル開発チェックです。依存ゼロの実行環境を維持するためTypeScriptはインストールせず、CI品質ゲートにも含めません。互換性のある`tsc`が既にある環境でのみ実行してください。リリース時の必須品質ゲートは`npm test`と`npm run audit:matrix`です。
