...
const finalizer=require('../../scripts/qa/finalize-phase-b-generation-13');

const script=path.join(
  core.ROOT,
  'scripts/qa/finalize-phase-b-generation-13.js'
);
...
const committed=
  childProcess.spawnSync(
    ...
    [
      'cat-file',
      '-e',
      'HEAD:reports/auto-gate/audit-locks/phase-b-generation-13.json'
    ],
    {cwd:core.ROOT}
  ).status===0;
...
try{
  ...
  const predecessorGeneration12=authorities.find(
    item=>item.document.generation===12
  );

  if(committed||issuedBeforeTest){
    const generation13=committed
      ? authorities.find(item=>item.document.generation===13)
      : {document:JSON.parse(fs.readFileSync(output,'utf8'))};

    test(
      'Generation 13 is discoverable',
      ()=>assert(generation13)
    );

    test(
      'Generation 13 predecessor is exact Generation 12',
      ()=>assert.deepStrictEqual(
        generation13.document.predecessor,
        lifecycle.identity(predecessorGeneration12.document)
      )
    );

    if(committed){
      test(
        'committed Generation 13 current integrity passes',
        ()=>assert.strictEqual(lifecycle.verifyCurrent().ok,true)
      );
    }else{
      test(
        'uncommitted issued Generation 13 verifies',
        ()=>assert.strictEqual(
          lifecycle.verifyCandidate(generation13.document).ok,
          true
        )
      );
    }
    ...
    test(
      'duplicate Generation 13 issuance fails',
    ...
    test(
      'candidate is Generation 13',
      ()=>assert.strictEqual(candidate.generation,13)
    );

    test(
      'predecessor is exact Generation 12',
      ()=>assert.deepStrictEqual(
    ...
    test(
      'approved Generation 12 successor baseline is pinned',
      ()=>assert.deepStrictEqual(
        [finalizer.approvedHead,finalizer.approvedTree],
        [
          '874243c0948e763b67b837768c5cdeabd4a85cd6',
          'c1be1b0ba5c3aa4aae8e99806e135d22459ea18d'
        ]
      )
    );
    ...
    test(
      ...
      ()=>{
        const target=path.join(
          core.ROOT,
          'independent-audit/unapproved-generation-13.tmp'
        );
        try{
...
console.log(
  `Generation 13 finalizer regressions: ${count}/${count}`
);
