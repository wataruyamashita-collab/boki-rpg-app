...
const committed=
  childProcess.spawnSync(
    ...
    [
      'cat-file',
      '-e',
      'HEAD:reports/auto-gate/audit-locks/phase-b-generation-11.json'
    ],
    {cwd:core.ROOT}
  ).status===0;
...
try{
  ...
  const generation10=authorities.find(
    item=>item.document.generation===10
  );

  if(committed||issuedBeforeTest){
    const generation11=committed
      ? authorities.find(item=>item.document.generation===11)
      : {document:JSON.parse(fs.readFileSync(output,'utf8'))};

    test(
      'Generation 11 is discoverable',
      ()=>assert(generation11)
    );

    test(
      'Generation 11 predecessor is exact Generation 10',
      ()=>assert.deepStrictEqual(
        generation11.document.predecessor,
        lifecycle.identity(generation10.document)
      )
    );

    if(committed){
      test(
        'committed Generation 11 current integrity passes',
        ()=>assert.strictEqual(lifecycle.verifyCurrent().ok,true)
      );
    }else{
      test(
        'uncommitted issued Generation 11 verifies',
        ()=>assert.strictEqual(
          lifecycle.verifyCandidate(generation11.document).ok,
          true
        )
      );
    }
    ...
    test(
      'duplicate Generation 11 issuance fails',
    ...
    test(
      'candidate is Generation 11',
      ()=>assert.strictEqual(candidate.generation,11)
    );

    test(
      'predecessor is exact Generation 10',
      ()=>assert.deepStrictEqual(
    ...
    test(
      'approved Generation 10 successor baseline is pinned',
      ()=>assert.deepStrictEqual(
        [finalizer.approvedHead,finalizer.approvedTree],
        [
          '33ad43d8960681f19a5283c24f9eaa5056e69540',
          '998e3d93994828a1ef8fe58989274e366c76d47b'
        ]
      )
    );
    ...
    test(
      ...
      ()=>{
        const target=path.join(
          core.ROOT,
          'independent-audit/unapproved-generation-11.tmp'
        );
        try{
...
console.log(
  `Generation 11 finalizer regressions: ${count}/${count}`
);
