...
const finalizer=require('../../scripts/qa/finalize-phase-b-generation-5');
const script=path.join(core.ROOT,'scripts/qa/finalize-phase-b-generation-5.js'),output=finalizer.output;
...
const committed=childProcess.spawnSync('git',['cat-file','-e','HEAD:reports/auto-gate/audit-locks/phase-b-generation-5.json'],{cwd:core.ROOT}).status===0;
const issuedBeforeTest=fs.existsSync(output);
try{
  const authorities=lifecycle.generationAuthorities(),generation4=authorities.find(item=>item.document.generation===4);
  if(committed||issuedBeforeTest){
    const generation5=committed?authorities.find(item=>item.document.generation===5):{document:JSON.parse(fs.readFileSync(output,'utf8'))};
    test('committed Generation 5 is discoverable',()=>assert(generation5));
    test('Generation 5 predecessor is exact Generation 4',()=>assert.deepStrictEqual(generation5.document.predecessor,lifecycle.identity(generation4.document)));
    if(committed)test('current integrity passes',()=>assert.strictEqual(lifecycle.verifyCurrent().ok,true));
    else test('uncommitted issued candidate verifies',()=>assert.strictEqual(lifecycle.verifyCandidate(generation5.document).ok,true));
    test('duplicate Generation 5 issuance fails',()=>assert.notStrictEqual(run().status,0));
    ...
    test('candidate is Generation 5',()=>assert.strictEqual(candidate.generation,5));
    test('predecessor is exact Generation 4',()=>assert.deepStrictEqual(candidate.predecessor,lifecycle.identity(generation4.document)));
    test('valid candidate passes',()=>assert.strictEqual(lifecycle.verifyCandidate(candidate).ok,true));
    test('generation skip fails',()=>{const mutant=structuredClone(candidate);mutant.generation=6;assert.strictEqual(lifecycle.verifyCandidate(mutant).ok,false);});
    ...
    test('approved D2 identity is pinned',()=>assert.deepStrictEqual([finalizer.approvedHead,finalizer.approvedTree],['1f54f4a564034bdd4a11edd64df29d53520fd209','0b97c43ee620da2d3e91556484be0d382eb9117f']));
    ...
    test('unexpected audit drift fails',()=>{const target=path.join(core.ROOT,'independent-audit/unapproved-generation-5.tmp');try{fs.writeFileSync(target,'unexpected\n');assert.notStrictEqual(run().status,0);}finally{fs.rmSync(target,{force:true});}});
...
console.log(`Generation 5 finalizer regressions: ${count}/${count}`);
