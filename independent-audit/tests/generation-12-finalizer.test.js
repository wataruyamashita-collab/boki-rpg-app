...
const finalizer=require('../../scripts/qa/finalize-phase-b-generation-12');

const script=path.join(
  core.ROOT,
  'scripts/qa/finalize-phase-b-generation-12.js'
);
...
const committed=
  childProcess.spawnSync(
    ...
    [
      'cat-file',
      '-e',
      'HEAD:reports/auto-gate/audit-locks/phase-b-generation-12.json'
    ],
    {cwd:core.ROOT}
  ).status===0;
...
try{
  ...
  const generation11=authorities.find(
    item=>item.document.generation===11
  );

  if(committed||issuedBeforeTest){
    const generation12=committed
      ? authorities.find(item=>item.document.generation===12)
      : {document:JSON.parse(fs.readFileSync(output,'utf8'))};

    test(
      'Generation 12 is discoverable',
      ()=>assert(generation12)
    );

    test(
      'Generation 12 predecessor is exact Generation 11',
      ()=>assert.deepStrictEqual(
        generation12.document.predecessor,
        lifecycle.identity(generation11.document)
      )
    );
    ...
    if(committed){
      ...
      test(
        'uncommitted issued Generation 12 verifies',
        ()=>assert.strictEqual(
          lifecycle.verifyCandidate(generation12.document).ok,
          true
        )
      );
    }
    ...
    test(
      'duplicate Generation 12 issuance fails',
    ...
    test(
      'candidate is Generation 12',
      ()=>assert.strictEqual(candidate.generation,12)
    );

    test(
      'predecessor is exact Generation 11',
      ()=>assert.deepStrictEqual(
    ...
    test(
      'approved Generation 11 successor baseline is pinned',
      ()=>assert.deepStrictEqual(
        [finalizer.approvedHead,finalizer.approvedTree],
        [
          '17a00d9601fc1b75a1f4a85f39762a3a1474c59d',
          'fa47af9eed47047e8777e1159bcf3062f4a2f211'
        ]
      )
    );
    ...
    test(
      ...
      ()=>{
        const target=path.join(
          core.ROOT,
          'independent-audit/unapproved-generation-12.tmp'
        );
        try{
...
console.log(
  `Generation 12 finalizer regressions: ${count}/${count}`
);
