...
const finalizer=require('../../scripts/qa/finalize-phase-b-generation-14');
...
const committed=
  childProcess.spawnSync(
    ...
    [
      'cat-file',
      '-e',
      'HEAD:reports/auto-gate/audit-locks/phase-b-generation-14.json'
    ],
    {cwd:core.ROOT}
  ).status===0;
...
try{
  ...
  const predecessorGeneration13=authorities.find(
    item=>item.document.generation===13
  );

  if(committed||issuedBeforeTest){
    const generation14=committed
      ? authorities.find(item=>item.document.generation===14)
      : {document:JSON.parse(fs.readFileSync(output,'utf8'))};

    test(
      'Generation 14 is discoverable',
      ()=>assert(generation14)
    );

    test(
      'Generation 14 predecessor is exact Generation 13',
      ()=>assert.deepStrictEqual(
        generation14.document.predecessor,
        lifecycle.identity(predecessorGeneration13.document)
      )
    );

    if(committed){
      test(
        'committed Generation 14 current integrity passes',
        ()=>assert.strictEqual(lifecycle.verifyCurrent().ok,true)
      );
    }else{
      test(
        'uncommitted issued Generation 14 verifies',
        ()=>assert.strictEqual(
          lifecycle.verifyCandidate(generation14.document).ok,
          true
        )
      );
    }
    ...
    test(
      'duplicate Generation 14 issuance fails',
    ...
    test(
      'candidate is Generation 14',
      ()=>assert.strictEqual(candidate.generation,14)
    );

    test(
      'predecessor is exact Generation 13',
      ()=>assert.deepStrictEqual(
    ...
    test(
      'approved Generation 13 successor baseline is pinned',
      ()=>assert.deepStrictEqual(
        [finalizer.approvedHead,finalizer.approvedTree],
        [
          '300a076882bc60a86720e66c556fc99572008064',
          '02ff85d760213397305a222ebdcf13eb53bba2e9'
        ]
      )
    );
    ...
    test(
      ...
      ()=>{
        const target=path.join(
          core.ROOT,
          'independent-audit/unapproved-generation-14.tmp'
        );
        try{
...
console.log(
  `Generation 14 finalizer regressions: ${count}/${count}`
);
