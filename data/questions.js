...
const QuestionData = {
  ...
  "J020": {
    ...
    "explanation": "【処理の根拠】\n小口現金係から受け取った支払報告に基づき、交通費・通信費・消耗品費を記録します。使った14,000円と同額の小切手を振り出して小口現金を補充するため、当座預金を14,000円減らします。\n【金額確認】借方合計・貸方合計はいずれも14,000円です。\n【試験のポイント】4,800＋3,200＋6,000＝14,000円です。",
    "learningRole": "core",
    "timelineRole": "main",
    "variantGroup": "小口現金精算"
  },
  ...
  "J121": {
    ...
    "explanation": "【処理の根拠】\n長期使用する業務用複合機は備品、商品以外の購入による後払い義務は未払金です。\n【金額確認】借方合計・貸方合計はいずれも300,000円です。",
    "learningRole": "transfer",
    "timelineRole": "timeless_case",
    "variantGroup": "固定資産購入"
  },
...
// 本試験の記帳能力を、金額だけでなく日付・摘要・元丁・相手勘定・残高まで測る独立形式。
const examLedgerVariant = (id, category, variant, question, materials, cells, labels, explanation) => ({
  id, type:'ledger', category, difficulty:4, chapter:10, scene:'本試験第2問・構造転移',
  story:'訓練画面とは異なる証憑配置から、記帳対象を選別して帳簿を完成する。', question, materials,
  table:{columns:['記入欄','解答'],rows:labels.map(item=>({item,amount:'入力'})),inputCells:Object.keys(cells),inputTypes:Object.fromEntries(Object.entries(cells).map(([key,value])=>[key,typeof value==='number'?'amount':'text'])),inputMetadata:Object.fromEntries(Object.keys(cells).map((key,index)=>[key,{label:labels[index],semanticType:/日$/.test(labels[index])?'date':typeof cells[key]==='number'?'amount':/科目/.test(labels[index])?'account':'text'}]))},
...
function independentlyDerivedTableCells(item) {
  ...
  if (item?.id === 'L033') {
    ...
    return {date:String(row.取得日),cost,depreciation,bookValue:cost-depreciation};
  }
  if (item?.id === 'F001') {
...
function validateSemanticQuestionData(questionData = QuestionData) {
  ...
  for (const [id, item] of entries) {
    ...
    const independentlyCalculated = item.type === 'journal' ? independentlyDerivedJournal(item) : independentlyDerivedTableCells(item);
    const ruleDerivedNumbers = new Set(item.type === 'journal' ? [...(independentlyCalculated?.debit || []), ...(independentlyCalculated?.credit || [])].map(row => row.amount) : Object.values(independentlyCalculated || {}).filter(value => typeof value === 'number'));
    for (const answer of new Set(answerNumbers(item))) {
      ...
      else if (answer > 0 && item.format !== 'exam-question-3' && !ruleDerivedNumbers.has(answer) && !canDerive(answer, inputs, text, item.type !== 'journal' || /[%％]|耐用年数|定額法|率/.test(text))) itemErrors.push(`正答金額 ${answer} を表示数値から導出できません`);
    }
